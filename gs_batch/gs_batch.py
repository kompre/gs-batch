...
def run_ghostscript(args):
    ...
    try:
        # print(f"Executing Ghostscript: {' '.join(args)}")
        process = subprocess.Popen(
            args, stdout=subprocess.PIPE, stderr=subprocess.STDOUT
        )
        # with click.progressbar(iter(process.stdout.readline, b""), length=56) as line:
        #     line
        total_length = None
        for line in iter(process.stdout.readline, b""):
            line = line.decode("utf-8")
            if line.startswith("Processing pages "):
                total_length = line.split(" ")[-1].replace(".", "")
                click.echo(f"{total_length=}")

            if total_length:
                with click.progressbar(
                    length=int(total_length), color=True, width=10
                ) as bar:
                    if line.startswith("Page "):
                        bar.update(int(line.split(" ")[-1]))
...
def process_file(file_info):
    ...
    gs_command = ["gs", "-sDEVICE=pdfwrite", "-o", temp_output_file]
...
def rename_output(temp_file, original_file, prefix, suffix, keep_smaller):
    ...
    click.echo(
        f"{'Original size':^{column_width}s} | {'New size':^{column_width}s} | {'Ratio':^{column_width}s} | {'keeping':^{column_width}s}"
    )
    ...
    click.echo(
        f"{human_readable_size(original_size):>{column_width}s} | {human_readable_size(new_size):>{column_width}s} | {ratio:{column_width}.3%} | { status:^{column_width}s}"
    )
...
def gs_batch(options, prefix, suffix, compress, pdfa, files, keep_smaller):
    ...
    file_tasks = [
        (pdf_file, command_parts, prefix, suffix, keep_smaller) for pdf_file in files
    ]
    ...
    #     # Base Ghostscript command
    #     gs_command = ["gswin64c", "-sDEVICE=pdfwrite", "-o", temp_output_file]
...
if __name__ == "__main__":
    ...
    result = runner.invoke(
        gs_batch, r'--compress=/ebook --prefix=".\\compressed\\" mat.pdf'
    )
    print(result.output)
